...
{elseif $product.paytype=='Once'}
    ...
    {/if}
    {if $admindata.access.viewOrdersPrices}{$product.m|price:$format} {$lang.Once} / {$product.m_setup|price:$format} {$lang.setupfee}{else} - {/if}
{else}
            ...
            <select class="inp" {if $caseif}disabled="disabled"{/if} name="{$name}" {$atributes}>
                {foreach from=$product item=p_price key=p_cycle}
                    {if $p_cycle == 'h' || $p_cycle == 'd' || $p_cycle == 'w' || $p_cycle == 'm' || $p_cycle == 'q' || $p_cycle == 's' || $p_cycle == 'a' || $p_cycle == 'b' || $p_cycle == 't' || $p_cycle == 'p4' || $p_cycle == 'p5'}
                        {if $p_price > 0}
                            <option value="{$p_cycle}" {if $cycle == $p_cycle}selected="selected" {/if} >{if $admindata.access.viewOrdersPrices}{$product.$p_cycle|price:$format}{/if} {$lang.$p_cycle} {assign value="`$p_cycle`_setup" var=p_setup}{if $admindata.access.viewOrdersPrices}{if $product.$p_setup>0}  + {$product.$p_setup|price:$format} {$lang.setupfee} {/if}{/if}</option>
                        {/if}
                    {/if}
                {/foreach}
            </select>
