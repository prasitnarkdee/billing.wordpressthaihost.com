{if $self_check}
        <table class="whitetable fs11 statustables" width="100%" cellspacing="0" cellpadding="3">
            <tr class="odd">
                ...
                <td width="12%">{if $self_check.php.ok}<b>{$self_check.php.version}</b>{else}<b class="err vtip_description" title="Error: HostBill requires PHP 7.2+, with lower versions it may not work stable.">{$self_check.php.version}</b>{/if}</td>
            ...
            <tr class="odd">
                ...
                <td >{if !$self_check.libsodium}<b>Installed</b>{else}<b class="warn vtip_description" title="Warning: Your PHP does not have Sodium support installed, its safer to store passwords with its enabled">Missing</b>{/if}</td>
                <td align="right">FileInfo</td>
                <td>{if !$self_check.fileinfo}<b>Enabled</b>{else}<b class="err vtip_description" title="ERROR: Your PHP is missing FileInfo extension - attachments may not work!.">Disabled</b>{/if}</td>
