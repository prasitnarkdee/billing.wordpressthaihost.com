    ...
    <form method="post" action="" id="sform1"><h3>{$spec.name}</h3>
        <div style="">
            ...
            {foreach from=$opconfig item=c key=k}
                ...
                {elseif $c.type=='countryflag'}
                    {if !$countryflaginit}
                        <link rel="stylesheet" media="screen" type="text/css" href="{$template_dir}js/countryflagpicker/countrySelect.min.css"/>
                        <script type="text/javascript" src="{$template_dir}js/countryflagpicker/countrySelect.min.js?v={$hb_version}"></script>
                        {assign var=countryflaginit value=true}
                    {/if}
                    <input id="country_selector" type="text" value="{$c.value}">
                    <input id="country_selector_hidden" type="hidden" name="opconfig[{$k}]" value="{$c.value}">
                    <script>
                        {literal}
                ...
                {elseif $c.type=='featurelist'}
                    ...
                    <div class="w250 winput" style="padding-top:7px !important; margin-bottom: 2px;">
                        ...
                        <input type="text" id="listn_{$k}" style="margin-bottom:5px"/>
                        <button id="btn_{$k}" style="padding: 3px 6px;margin: 2px;">{$lang.Add}</button>
                    </div>
                    <script type="text/javascript">
                        function list_{$k}() {literal}{{/literal}
                            $('#ulist_{$k}').html($($('#list_{$k}').val()).filter('li').each(function () {literal}{
                                {/literal}$(this).append('<a class="fs11 editbtn" style="margin-left:5px" href="#" onclick="remol_{$k}(this); return false;">{$lang.delete}</a>'){literal}}{/literal}));
                            {literal}}{/literal}
                        function remol_{$k}(el) {literal}{{/literal}
                            ...
                            {literal}}{/literal}
                        list_{$k}();
                        $('#btn_{$k}').click(function () {literal}{
                            {/literal}$('#list_{$k}').val($('#list_{$k}').val() + '<li>' + $('#listn_{$k}').val() + '</li>');
                            list_{$k}();
                            return false;{literal}}{/literal});
                    </script>
                {elseif $c.type=='premade'}
                            ...
                            {foreach from=$c.value item=premade}
                                {if $premade.name}
                                    <li>
                                        ...
                                        <a href="#" class="fs11" onclick="return premade.del('{$k}','{$premades}');">
                                            delete</a>
                                    </li>
                                {/if}
                            {/foreach}
                    ...
                    {if $premade.name}
                        ...
                        <div id="old_{$k}_{$premades}" style="display: none;">
                            ...
                            <input type="text" value="{$premade.name}" name="opconfig[{$k}][{$premades}][name]" class="w250" style="margin-bottom:5px"/>
                            <label style="font-weight: normal">Description</label>
                            <textarea name="opconfig[{$k}][{$premades}][description]" class="w250" style="margin-bottom:5px">{$premade.description}</textarea>
                            <label style="font-weight: normal">Target package</label>
                            <input value="{$premade.package}" type="text" name="opconfig[{$k}][{$premades}][package]" class="w250" style="margin-bottom:5px"/>
                    ...
                    <div id="new_{$k}" style="display: none;">
                        {counter name=premades}
                        <label style="font-weight: normal">Name</label>
                        <input type="text" name="opconfig[{$k}][{$premades}][name]" class="w250" style="margin-bottom:5px"/>
                        <label style="font-weight: normal">Description</label>
                        <textarea name="opconfig[{$k}][{$premades}][description]" class="w250" style="margin-bottom:5px"></textarea>
                    ...
                    <div id="itool_{$k}" style="display: none;">
                        ...
                        <label>
                            <button id="btn_{$k}" onclick="return premade.save('{$k}', '{$premades}');">{$lang.Add}</button>
                        </label>
                    </div>
                ...
                {elseif $c.type=='colorpicker'}
                    ...
                    <script type="text/javascript">
                        $('#colorSelector_{$k}').ColorPicker({literal}{
                            ...
                            livePreview: true, color: {/literal}'{$c.value}'{literal}}{/literal});
                    </script>
                {/if}
                <div class="clear"></div>
...
<div class="dark_shelf dbottom">
    ...
    <div class="right">
        <span class="bcontainer "><a class="new_control greenbtn" href="#"
                                     onclick="HBOpConfig.fsubmit();return false"><span>{$lang.savechanges}</span></a></span>
        <span>{$lang.Or}</span>
        <span class="bcontainer"><a href="#" class="submiter menuitm"
                                    onclick="$(document).trigger('close.facebox');return false;"><span>{$lang.Close}</span></a></span>
    </div>
    <div class="clear"></div>
</div>
