{if $show=='pricingopt'}
    ...
    {/if}
    <input id="pricing_overide_switch" type="checkbox" value="1" name="bundle_fixedprice"
           {if $bundle_config.fixedprice}checked="checked"{/if} onchange="bundle_pricingopt(this); return false;"/>
    Set fixed price
    <div id="pricing_overide" {if !$bundle_config.fixedprice}style="display: none"{/if} >
        ...
        {foreach from=$bundle_billingmodels item=bm name=boptloop}
            <a href="#"
               ...
               onclick='return switch_t2(this,"{$bm}");'>
                {if $lang[$bm]}{$lang[$bm]}
                {else}{$bm}
                {/if}
            </a>
            <input type="radio" value="{$bm}" name="bundle_paytype" {if $product.paytype==$bm}checked="checked"{/if}
    ...
    {if $bundle_config.discount || ($product.paytype!='Free' && $showpricebox)}
        <div id="pricing_dynamo" {if $bundle_config.fixedprice}style="display: none"{/if} class="p5">
            {if $product.paytype=='Once'}
                <input type="checkbox" value="Once" name="dynamic_paytype" disabled="disabled" checked="checked"
                       style="display:none"/>
            ...
            {else}
                <input type="checkbox" value="BundleRegular" name="dynamic_paytype" disabled="disabled"
                ...
                <span class="editbtn_flash">
                    {if !$bundle_config.discount}
                        ...
                        <a href="#" class="editbtn"
                           onclick="$(this).parent().hide().next().show();return false;">{$lang.Add}</a>

{else}
                        {if $bundle_config.discount.code}
                        <a href="?cmd=coupons&action=edit&id={$bundle_config.discount.id}"
                           target="_blank">{$bundle_config.discount.code} </a>
                    {/if}
                    ...
                    {/if}
                        ...
                        <a href="#" class="editbtn"
                           onclick="$(this).parent().next().remove(); bundle_discount(); return false;">{$lang.Remove}</a>
                    {/if}
                </span>
                    <span style="display:none">
                    {if $coupons}
                        <select class="inp" name="discount_coupon"
                                onchange="if($(this).val() == 0) $(this).hide().next().show();">
                            <option value="">------</option>
    ...
    <div class="bundle-item-config" id="config{$details.id}_{$bundleid}_{$type}">
        {if $type == 'Product'}
            <div class="form-group row">
                <div class="col-md-3">{$lang.billingcycle}</div>
                <div class="col-md-9">
                    {if $details.paytype=='Free'}
                        {$lang.freeproduct}
                        <input name="product_cycle[{$bundleid}][]" value="free" type="hidden"/>
                    {elseif $details.paytype=='Once'}
                        <input name="product_cycle[{$bundleid}][]" value="once" type="hidden"/>
                        {$details.m|price:$currency} {$lang.Once} / {$details.m_setup|price:$currency} {$lang.setupfee}
                    {else}
                        <select name="product_cycle[{$bundleid}][]" multiple class="chosen">
                            {foreach from=$details item=p_price key=p_cycle}
                                {if $p_cycle == 'd' || $p_cycle == 'w' || $p_cycle == 'm' || $p_cycle == 'q' || $p_cycle == 's' || $p_cycle == 'a' || $p_cycle == 'b' || $p_cycle == 't' || $p_cycle == 'p4' || $p_cycle == 'p5'}
                                    {if $p_price > 0}
                                        <option value="{$p_cycle}" {if $draft.services[$p_cycle]}selected{/if} >
                                            {$p_price|price:$currency}
                                            {$lang.$p_cycle}
                                            {assign value="`$p_cycle`_setup" var=p_setup}
                                            {if $details.$p_setup>0}
                                                + {$details.$p_setup|price:$currency} {$lang.setupfee}
                                            {/if}
                                        </option>
                                    {/if}
                                {/if}
            ...
            *}
            {if $details.addons}
                {foreach from=$details.addons item=addon key=addon_id}
                    <div class="form-group row">
                        <div class="col-md-3">{$addon.name}</div>
                        <div class="col-md-9">
                            <input class="inp" type="text"
                                   onkeypress="if(event.charCode != 0 && (event.charCode < 48 || event.charCode > 57)) return false;"
                                   size="2" value="{$addon.qty}" name="addons[{$bundleid}][{$addon_id}][qty]"/> x
                    ...
                    {if $c.items}
                        <div class="form-group row">
                            <label class="col-md-2 pull-right">
                                <input class="form-disabler"
                                       {if $submit.bundle_custom[$bundleid][$c.id]}checked="checked"{/if}
                                       type="checkbox" value="1" name="bundle_custom[{$bundleid}][{$c.id}]"
                                       onchange="bundle_optionalconfig(this);">
                                Display in cart
                            </label>
                            <div class="col-md-3"><span>{$c.name} </span></div>
                            <div class="col-md-7 details_cf_{$kk}" default="{$c.config.initialval}">
        ...
        {elseif $type=='Domain'}
            {if $details.periods}
                <div class="form-group row">
                    <div class="col-md-3">{$lang.Period}</div>
                    <div class="col-md-9">
                        <select name="domain_period[{$bundleid}][]" multiple class="chosen">
                            {foreach from=$details.periods item=period}
                                <option value="{$period.period}" {if $draft.domains[$period.period]}selected{/if} >
                                    {$period.period}
                                    {if $period.period == 1} {$lang.Year}{else} {$lang.Years}{/if}
                                </option>
                            {/foreach}
                        </select>
                    </div>
                    ...
                    {foreach from=$details.forms item=c key=kk}
                        {if $c.items}
                            <div class="form-group row">
                                ...
                                <div class="col-md-9 product_cf_{$kk}" default="{$c.config.initialval}">
                                    {include file=$c.configtemplates.accounts}
                                </div>
                            </div>
                        {/if}
            ...
            <div class="form-group row">
                ...
                <div class="col-md-9">
                    {if $details.paytype=='Free'}
                        {$lang.freeproduct}
                    {elseif $details.paytype=='Once'}
                        {$details.m|price:$format} {$lang.Once} / {$details.m_setup|price:$format} {$lang.setupfee}
                    {else}
                        <select name="addon_cycle[{$bundleid}][]" multiple class="chosen">
                            {foreach from=$details item=p_price key=p_cycle}
                                {if $p_cycle == 'd' || $p_cycle == 'w' || $p_cycle == 'm' || $p_cycle == 'q' || $p_cycle == 's' || $p_cycle == 'a' || $p_cycle == 'b' || $p_cycle == 't' || $p_cycle == 'p4' || $p_cycle == 'p5'}
                                    {if $p_price > 0}
                                        <option value="{$p_cycle}" {if $draft.addons[$p_cycle]}selected{/if}>
                                            {$p_price|price:$currency}
                                            {$lang.$p_cycle} {assign value="`$p_cycle`_setup" var=p_setup}
                                            {if $details.$p_setup>0}
                                                + {$details.$p_setup|price:$currency} {$lang.setupfee}
                                            {/if}
                                        </option>
