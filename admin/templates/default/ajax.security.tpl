{if $action != 'fastban' && $action != 'fastunban' && $action != 'iplog' && $action != 'password'}
...
{if $security_rules}
    {foreach from=$security_rules item=rule}
        <tr>
            ...
            <td width="30%">
                <div id="descrulespan-{$rule.id}">
                    {if $rule.description}
                        {$rule.description}
                    {else}{/if}
                    <a href="#" onclick="$('#descruleform-{$rule.id}').show();$('#descrulespan-{$rule.id}').hide();return false;">Change</a>
                </div>

                <form onsubmit="return desc_rule(this);" id="descruleform-{$rule.id}" style="display: none;" method="POST">
                    <input name="id" value="{$rule.id}" type="hidden"/>
                    <input name="make" value="desc_rule" type="hidden"/>
                    <input type="text" name="description" value="{$rule.description}">
                    <a href="#" onclick="$('#descruleform-{$rule.id}').closest('form').submit();return false;">Save</a>
                    {securitytoken}
                </form>
            </td>
            <td>
                {if $rule.action == 'allow'}
                    <a href="?cmd=security&make=toggle_rule&id={$rule.id}" onclick="return toggle_rule(this)" class="editbtn" data-enabled="{if $rule.options & 2}true{/if}"
                       title="{if $rule.options & 2}Enable{else}Disable{/if} automatic bans for users coming from this address">
                        {if $rule.options & 2}Disabled{else}Enabled{/if}
                    </a>
                {else}-
                {/if}
            </td>
...
{elseif $banned_ips}
    {foreach from=$banned_ips item=ban_ip}
        <tr {if $highlight == $ban_ip.id}class="checkedRow"{/if}>
            <td>
                {$ban_ip.ip}<a href="#{$ban_ip.ip}"></a>
            </td>
            <td>{if $ban_ip.expires == '2999-01-01 00:00:00'}Never{else}{$ban_ip.expires|dateformat:$date_format}{/if}</td>
            <td>{$ban_ip.reason}</td>
...
{elseif $api_access}
    {foreach from=$api_access item=apia}
        <tr>
            ...
            <td>{$apia.api_id}</td>
            <td>{$apia.api_key}</td>
            <td><a href="?cmd=security&action=apiacl&id={$apia.id}">Allowed functions</a></td>
            <td></td>
            <td width="20">{if !$apia.flags}<a href="?cmd=security&action=apiaccess&make=remove_api&id={$apia.id}" onclick="{literal}return removeAPI($(this).attr('href')){/literal}" class="delbtn">{$lang.Remove}</a>{/if}
            </td>
        </tr>
    {/foreach}
{else}
...
{elseif $action == 'iplog'}
    {if $admin_list}
        ...
        <tr>
            <td><span title="{$admin.username}">{$admin.firstname} {$admin.lastname}</span> {*firstname, lastname, email, emails(1|0)*}</td>
