    ...
    <div class="newhorizontalnav" id="newshelfnav">
        <div class="list-1">
            <ul>
                ...
                <li {if $action=='ratings'}class="active"{/if}>
                    <a href="?cmd={$cmd}&action=ticket_ratings"><span>{$lang.ticket_ratings}</span></a>
                </li>
            ...
            <div class="subm1 haveitems" style="display: block;{if $action =='config' || !$enableddepts}height:0px{/if}">
                {if $action !='config' && $enableddepts}
                    <form action="?cmd={$cmd}&action={$action}{if $type}&type={$type}&id={$id}{/if}" id="rateform" method="post" >
                        {if $action == 'default' || $action == 'staff' || $action == 'departments'}
                            <ul>
                                <li class="list-2elem {if !$per_ticket}active{/if}"><a href="?cmd={$cmd}&action={$action}"><span>{$lang.per_reply}</span></a></li>
                                <li class="list-2elem {if $per_ticket}active{/if}"><a href="?cmd={$cmd}&action={$action}&per_ticket=true"><span>{$lang.per_ticket}</span></a></li>
                            </ul>
                        {/if}
                    ...
                    <thead>
                        <tr>
                            {if $action != 'ticket_ratings'}
                                ...
                                {/if}
                            {else}
                                <th><a href="?cmd={$cmd}&action={$action}&orderby=ticket_number|ASC"  class="sortorder">Ticket Number</a></th>
                                <th><a href="?cmd={$cmd}&action={$action}&orderby=rating|ASC"  class="sortorder">Rating</a></th>
                                <th><a href="?cmd={$cmd}&action={$action}&orderby=date|ASC"  class="sortorder">Date</a></th>
                                <th><a href="?cmd={$cmd}&action={$action}&orderby=comment|ASC"  class="sortorder">Comment</a></th>
                            {/if}
                        </tr>
                    </thead>
                    <tbody id="updater">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <select class="inp" name="rating_scale" id="newscale">
                                    <option {if $rate == 3}selected="selected"{/if} value="3">3</option>
    ...
    {foreach from=$ratings item=rating}
        <tr>
            {if $action != 'ticket_ratings'}
                ...
                {/if}
            {else}
                <td width="10%">{if $rating.ticket_number}<a href="?cmd=tickets&action=view&num={$rating.ticket_number}">{$rating.ticket_number}</a>{else}-{/if}</td>
                <td width="7%">{$rating.rating}</td>
                <td width="13%">{$rating.date|dateformat:$date_format}</td>
                <td width="70%">{$rating.comment}</td>
            {/if}
        </tr>
    {foreachelse}
        <tr>
            <td colspan="{if $action !='details'}6{else}{if $action == 'ticket_ratings'}4{else}5{/if}{/if}">
