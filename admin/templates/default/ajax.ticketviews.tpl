...
{elseif $display=='trow'}
    ...
    {elseif $col == 32 || $col == 1 || $col == 2097152}{* Number  or ID*}
            ...
            <ul class="view-filters clearfix">
                {foreach from=$filters item=filtr key=field name=loop}
                    ...
                    <li>
                        ...
                        {if $filtr.type == 'tags'}
                            <a href="#" id="tagdescr" class="vtip_description" title="You can use &quot;and&quot;, &quot;or&quot;, &quot;not&quot; keywords when filtering with tags, default is &quot;and&quot;, example: <br> &bullet;&nbsp;tag1 tag2 or tag3 &raquo; (tag1 and tag2) or tag3<br><br>You can use the <b>untagged</b> tag for tickets that have no tags"></a>
                        {/if}
                        </span>
                        ...
                        {elseif $filtr.type == 'tags'}
                            ...
                            {*}
                        {else}
                            <input class="inp inp_{$filtr.type}" {if $filtr.type} type="{$filtr.type}"{else}type="text"{/if} value="{$view.filters.$field|escape}" name="view_filter[{$field}]" {foreach from=$filtr item=attv key=attn}{if $attn!='type' && $attn!='name'}{$attn}="{$attv}"{/if}{/foreach}/>
                        {/if}
                    </li>
                    {/if}
                {/foreach}
