var HBOperator = {
    ...
    banVisitorPrompt: function (name, id) {
        this.open_modal(this.renderTemplate('visitorban_title', {visitor_name: name}), this.renderTemplate('visitorban_content', {
            visitor_name: name,
            id: id
        }));
        return false;
    ...
    banVisitor: function (id) {
        this.close_modal();
        var self = this;
        $.post('?cmd=hbchat&action=banvisitor', {id: id}, function (data) {
            self.parseReponse(data);
            if (data && data.success) {
    ...
    update_chat_details: function () {
        ...
        $.post('?cmd=hbchat&action=updatechat', {
            chat_id: chat_id[1],
            visitor_name: vname,
    ...
    playSound: function (which) {
        if (!this.options.sound && which == 'newmsg') {
            return;
        }
