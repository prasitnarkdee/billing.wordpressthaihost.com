    ...
    <tr id="line_{$item.id}">
        ...
        <td class="acenter">
            <input name="item[{$item.id}][qty]" value="{$item.qty}"
                   size="8" class="foc invitem invqty" style="text-align:center"/>
        </td>
        <td class="acenter">
            <input type="hidden" name="item[{$item.id}][taxed]" value="{$item.taxed}" />
            <input type="text" name="item[{$item.id}][tax_rate]"
                   size="4" value="{$item.tax_rate}" class="invitem invtaxrate" style="text-align:right" />
        </td>
        <td class="acenter">
            <input name="item[{$item.id}][amount]" value="{$item.amount}"
                   size="16" class="foc invitem invamount" style="text-align:right"/>
        </td>
        <td class="aright">
