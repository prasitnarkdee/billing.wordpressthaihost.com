...
function handlePlaceholderPrice(selectTldEl) {
    var option = $(selectTldEl).find(':selected');
    var row = $(selectTldEl).closest('tr');
...
function bindMarginOptions() {
    ...
    $('.margin_type').on('change', function () {
        ...
        handlePlaceholderPrice($(this).closest('tr').find('.tld_select'));
    }).each(function () {
        $(this).trigger('change');
    });
    ...
    $('.remove_tld_settings').unbind('click').bind('click', function (e) {
        e.preventDefault();
...
function addTldSetting() {
    var itt = parseInt($('.tld_settings_body tr').length) + 1;
    var template = $('#tld-setting-template').html();
    template = template.replace(/__itt/g, itt);
    $('.tld_settings_body').append(template);
...
$(function () {
    ...
    $(server_chosen).chosenedge({
        ...
        $.post('?cmd=importaccounts&action=verifySupportTLD', {id: this.value}, function (data) {
            warnI.hide();
            warnP.hide();
            options.hide();
