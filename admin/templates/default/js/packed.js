...
function bindInvoiceEvents() {
    ...
    $('.setStatus').unbind('click').click(function() {
        ...
        } else if(action == 'AddPrintQueue' || action == 'RemovePrintQueue' || action == 'MarkAsPrinted' || action == 'MarkAsNotPrinted') {
            $.post('?cmd=invoices&action=changething&make=printqueue', {
                id: invoice_id,
                type: action
            }, function (data) {
                var resp = parse_response(data);
                if (resp != false && resp != null){
                    $('.invoice_flag, .print-status').hide();
                    if (action == 'AddPrintQueue') {
                        $('.flag2, .flag3, .print-status-notprinted').show();
                    } else if (action == 'RemovePrintQueue') {
                        $('.flag1, .flag3').show();
                    } else if (action == 'MarkAsPrinted') {
                        $('.flag4, .print-status-printed').show();
                    } else if (action == 'MarkAsNotPrinted') {
                        $('.flag2, .flag3, .print-status-notprinted').show();
                    }
                }
            });
...
function bindTicketEvents() {
    ...
    $('.setStatus').dropdownMenu({}, function(action, el, pos, valx) {
        action = action.substr(action.lastIndexOf('/') + 1);
        var color = $(pos.target).data("color");
        if(color === "000000")
            color = false;
        ...
        if (action.lastIndexOf('status|') != -1) {
            ...
            $.post('?cmd=tickets&action=menubutton&make=setstatus', {
                ...
                if (resp != false && resp != null) {
                    ...
                    if(color == false)
                        $("#ticket_status").removeAttr("style");
                    else
                        $("#ticket_status").css("color", "#"+color);
                    $('#ticket_status').attr({
                        'class': action
                    });
    ...
    var x = $('.ticketmsg:gt(0)').not($('#order_replies').val() + ', .tpinned');
    if (x.length > 3) {
...
var AdminNotes = {
    ...
    edit: function (id) {
        var value = $('#AdmNotes textarea:visible:last').val(),
            flags = new Array();
        $('#AdmNotes input[name="flags['+id+'][]"]').each(function() {
            if ($(this).prop('checked')) {
                flags.push($(this).val());
            }
        });
        if (value && value.length)
            $.post($('#notesurl').attr('href'), {make: 'editNote', note: value, noteid: id, flags: flags}, AdminNotes.ajax);
        return false;
    },
    init: function() {
