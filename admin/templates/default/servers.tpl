...
<table border="0" cellspacing="0" cellpadding="0" width="100%" id="content_tb" {if $currentfilter && !$details.manual}class="searchon"{/if}>
    <tr>
        <td ><h3>{$lang.manapps}</h3></td>
        <td  class="searchbox">
            {if $action == 'default'}
            <div id="hider2" style="text-align:right">
                <a href="?cmd=servers&action=getadvanced{if $list_type}&list={$list_type}{/if}" class="fadvanced">{$lang.filterdata}</a> 
                <a href="?cmd=servers&resetfilter=1" {if $currentfilter}style="display:inline"{/if}  class="freseter">{$lang.filterisactive}</a>
            </div>
            ...
            <div id="bodycont" style="">
                {if $action=='add'}
                    <form method="post" action="" id="serverform">
                        ...
                        <div class="blu"> 
                            <a href="?cmd=servers" ><strong>{$lang.manapps}</strong></a> 
                            {if $group}&raquo; <a href="?cmd=servers&action=group&group={$group.id}"><strong>{$group.name}</strong></a>
                            {/if} 
                            &raquo; <strong>{$lang.addnewapp}</strong>

                        </div>
                        {if $group}
                            <input type="hidden" name="default_module" value="{$group.module}" id="default_module"  />
                        {/if}
                        <div class="lighterblue form-horizontal" style="padding: 5px 20px">
                            <table border="0" cellpadding="3" cellspacing="0" width="100%">
                                <tr>
                                    {if !$group}
                                        ...
                                        <td class="row no-gutter">
                                            <div class="col-md-6">
                                                <select name="default_module" onchange="loadMod(this)"
                                                        id="modulepicker" class="inp chosen"">
                                                <option value="0" {if !$setmodule}selected="selected"{/if} >
                                                    {$lang.none}
                                                </option>
                                                    {foreach from=$modules item=mod}
                                                        {if $mod.id!='-1'}
                                                            <option value="{$mod.id}"
                                                                    {if $setmodule==$mod.id}selected="selected"{/if}>{$mod.modname}</option>
                                                        {/if}
                                                    {/foreach}
                                                <option value="new" style="font-weight:bold">
                                                    Show non-activated modules
                                                </option>
                                                </select>
                                            </div>
                                            {literal}
                                ...
                                <tr>
                                    ...
                                    <td class="row no-gutter">
                                        ...
                                        <div class="col-md-6">
                                            &nbsp;
                                            <a class="btn btn-default" href="#" id="testing_button"
                                               {if !$group && !$setmodule}style="display: none"{/if} onclick="testConfiguration();
                                                return false;">
                            ...
                            <table border="0" cellpadding="6" cellspacing="0" width="100%">           
                                <tr>
                                    ...
                                    <td style="white-space: nowrap; width: 500px">
                                        ...
                                        {if $can_sso}
                                            <span>
                                                <a class="btn btn-sm btn-default" href="?cmd=servers&action=login_to_panel&id={$server.id}&security_token={$security_token}" target="_blank">{$lang.login_to_panel}</a>
                                            </span>
                                        {/if}
                                    </td>
                                    <td id="testing_result">
                                ...
                                <div class="panel panel-default" >
                                    <div class="panel-heading" role="tab" >
                                        <h4 class="panel-title">
                                            <a href="#collapseListGroup3" class="" role="button" data-toggle="collapse"  aria-controls="collapseListGroup2"> {$lang.moreoptions} </a> </h4> </div>
                                    <div class="panel-collapse collapse" role="tabpanel" id="collapseListGroup3" aria-labelledby="collapseListGroupHeading3"  >
                        ...
                        <table width="100%" cellspacing="0" cellpadding="3" border="0" class="table glike">
                            <thead>
                                <tr>
                                    ...
                                    <th ><a href="?cmd={$cmd}&action=group&group={$group.id}&orderby=name|ASC"  class="sortorder">{$lang.Name}</a></th>
                                    <th >{if $group.type=='Domain'}{$lang.Domains}{else}{$lang.Users}{/if}</th>
                                    <th>{if $group.type!='Domain'}<a href="?cmd={$cmd}&action=group&group={$group.id}&orderby=ip|ASC"  class="sortorder">IP</a>{/if}</th>
                                    <th>{if $group.type!='Domain'}<a href="?cmd={$cmd}&action=group&group={$group.id}&orderby=host|ASC"  class="sortorder">{$lang.Host}</a>{/if}</th>
                                    <th width="120"><a href="?cmd={$cmd}&action=group&group={$group.id}&orderby=monthly_cost|ASC"  class="sortorder">{$lang.monthlycosts}</a></th>
                                    <th></th>
                                    <th width="100"></th>
                                    <th width="20"></th>
                                    <th width="100"></th>
                ...
                {elseif $action=='default'}
                    ...
                    {else}
                        ...
                        <table width="100%" cellspacing="0" cellpadding="3" border="0" class="table glike">
                            ...
                            <tbody id="psummary">
                                <tr>
                                    <th><a href="?cmd=servers&action=add" class="btn btn-success btn-xs">{$lang.addnewapp}</a></th>
                                    <th colspan="4" style="text-align: right">
                                        {$lang.showing} <span id="sorterlow">{$sorterlow}</span> - <span id="sorterhigh">{$sorterhigh}</span> {$lang.of} <span id="sorterrecords">{$sorterrecords}</span>
                                    </th>
                                </tr>
