{foreach from=$c.items item=cit}
    <input name="custom[{$kk}][{$cit.id}]" data-variable="{$c.variable}" size="8" value="{$c.qty}"  {if $fieldlogic}class="custom_field_{$c.copy_of}"{/if} {if $cmd == 'orders' && $c.options & 1}required{/if}/>
