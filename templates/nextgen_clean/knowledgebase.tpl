    ...
    <div class="p19">
        ...
        {if $vote_article}
            <div id="knowledgebase_vote" class="spacing">
                {include file="ajax.knowledgebasevote.tpl"}
            </div>
        {else}
            <br>
        {/if}
        {if $relations}
            <b>{$lang.related_articles}:</b>
            <ul>
                {foreach from=$relations item=relate}
                    <li>
                        <a href="?cmd=knowledgebase&action=article&id={$relate.id}" >{$relate.title}</a>
                    </li>
                {/foreach}
            </ul>
        {/if}
