...
<div class="row flex-container bordered-section">
    ...
    <div class="span3 left-content flex-box-1">
        ...
        <ul class="leftmenu">
            <li {if !$widget}class="current"{/if}>
                <a href="{$ca_url}clientarea/domains/{$details.id}/{$details.name}/"><span
                            class="home">{$lang.domdetails}</span></a>
            </li>
            {foreach from=$widgets item=widg name=cst}
                {if $widg.name!='reglock' && $widg.name!='nameservers'  && $widg.name!='autorenew' } {if $widg.name=='idprotection' && $details.offerprotection && !$details.offerprotection.purchased}
                    {continue}
                {/if}
            ...
            {if $custom} {foreach from=$custom item=btn name=cst}
                <li {if $smarty.foreach.cst.last}class="last"{/if}>
                    <a href="#" onclick="$('#cbtn_{$btn}').click();return false;">{$lang.$btn}</a>
                </li>
            {/foreach}
            {/if}
            {if $cancanel}
                <li>
                    <a href="{$ca_url}clientarea/domains/{$details.id}/{$details.name}/&cancel">
                      <span>
                          {$lang.cancelrequest}
                      </span>
                    </a>
