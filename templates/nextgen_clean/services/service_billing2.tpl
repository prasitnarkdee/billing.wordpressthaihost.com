<div class="p19 serviceinfo">
    <div class="row">
        ...
        <div class="span8 mb15">
            ...
            <h4>{$service.catname} - {$service.name}
                {if $upgrades && $upgrades!='-1'}
                    <small><a href="{$ca_url}clientarea/services/{$service.slug}/{$service.id}/&make=upgrades&upgradetarget=service"
                              class="lmore">{$lang.UpgradeDowngrade}</a></small>
                {/if}
            </h4>
        </div>
        ...
        {if $service.showbilling}
            ...
            {if $service.commitment_date && $service.commitment_date != '0000-00-00'}
                <div class="span4 mb15">
                    <h3>{$lang.commitmentdate}</h3>
                    <h4>{$service.commitment_date|dateformat:$date_format}</h4>
                </div>
            {/if}
        {/if}
    </div>
