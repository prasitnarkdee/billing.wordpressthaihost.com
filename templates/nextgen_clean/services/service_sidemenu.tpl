...
<ul class="leftmenu">
    ...
    {foreach from=$widgets item=widg}
        <li {if $widget.name==$widg.name}class="current"{/if}>
            <a  href="{$ca_url}clientarea/services/{$service.slug}/{$service.id}/&widget={$widg.name}{if $widg.id}&wid={$widg.id}{/if}"><span class="{$widg.name}">
    ...
    {if $service.status=='Active'}
        ...
        {else}
            <li>
                <a href="{$ca_url}clientarea&action=services&service={$service.id}&cid={$service.category_id}&customfn={$command}"
                   onclick="return callcustomfn('{$command}',{$service.id}, '#_ocustom', true)"><span
                            class="cog">{if $lang.$command}{$lang.$command}{else}{$command}{/if}</span>
                </a>
            </li>
        {/if}
    ...
    {foreach from=$haveaddons item=newaddon}
        <li><a class="no-restriction"
               href="{$ca_url}cart&amp;action=add&amp;cat_id=addons&amp;id={$newaddon.id}&amp;account_id={$service.id}&amp;addon_cycles[{$newaddon.id}]={$newaddon.paytype}">
                <span class="plus">{$lang.Add} {$newaddon.name}</span></a>
        </li>
    {/foreach}

    {if $service.status!='Terminated' && $service.status!='Cancelled' && $service.showcancel}
        <li class="last"><a href="{$ca_url}clientarea&action=services&service={$service.id}&cid={$service.category_id}&cancel" style="color:red"><span class="cancel">{$lang.cancelrequest}</span></a></li>
    {/if}
