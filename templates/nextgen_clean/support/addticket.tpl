                ...
                <tr>
                    <td valign="middle" colspan="2">
                        ...
                        {foreach from=$depts item=dept name=fff}
                            <div class="dept_{$dept.id}" {if $submit.dept_id && $submit.dept_id==$dept.id} style="display:block" {elseif !$submit.dept_id && $smarty.foreach.fff.first}  style="display:block"  {else} style="display:none"{/if}>{$dept.description}</div>
                        {/foreach}
                        {foreach from=$depts item=dept name=fff}
                            {if $depts_avg[$dept.id]}
                            <div class="dept_{$dept.id}" {if $submit.dept_id && $submit.dept_id==$dept.id} style="display:block" {elseif !$submit.dept_id && $smarty.foreach.fff.first}  style="display:block" {else} style="display:none" {/if}>
